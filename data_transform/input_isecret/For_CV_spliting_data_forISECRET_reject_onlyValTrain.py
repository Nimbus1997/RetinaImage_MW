...
reject_path = "/root/jieunoh/ellen_data/reject_crop_512"
new_path="/root/jieunoh/ellen_data/0_isecret_input_reject/isinput_eyeq_total_reject"
...
# reject -> A group
rtotal_dataset = len(os.listdir(reject_path))

rtrainN = int(float(rtotal_dataset)/float(trainlR+vallR)*float(trainlR))
rvalN = rtotal_dataset - rtrainN
...
print("Low qualtiy(usable) # of dataset in (train : val) : (",
      ltrainN, ", ", lvalN, ")")
print("Reject qualtiy # of dataset in (train : val) : (",
      rtrainN, ", ", rvalN, ")")
...
print("[low quality (usable) end]==================================\n\n")


# reject
reject_quality_block_size=rtotal_dataset/total_dataset
print("LQ (reject)------------------------------------------------------")
for index, path in enumerate(path_list):
    print("--[",path,"]-----------------------------------------")
    val_start = reject_quality_block_size*index

    i=0
    val_count=0
    train_count=0

    print(str(val_start),"/", str(val_start+reject_quality_block_size))

    for rlq in tqdm(sorted(os.listdir(reject_path))):
        if imageformat in rlq:
            source_path=reject_path+'/'+rlq

        #2. real lq image 저장하기 ------------------
            # val = test
            if (i>=val_start) and (i<(val_start+reject_quality_block_size)): 
                copy_path_val =path+"/eyeq/val/"+group2[1]         
                shutil.copy(source_path,copy_path_val)
